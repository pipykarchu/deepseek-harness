...
- **One-shot rendering by design** — the UI waits for the boot settle and the door gesture; a single entry failure keeps the loading page with a loud per-entry report, no partial availability (progressive rendering returns with its own project).
