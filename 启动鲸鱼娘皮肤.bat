...
REM ---- 1) 定位 node（多候选，找不到就用 PATH）----
if not defined DSH_NODE_DIR (
  if exist "D:\DEEPSEEK harness\node\node.exe" set "DSH_NODE_DIR=D:\DEEPSEEK harness\node"
...
REM ---- 2) 定位 harness（多候选，找不到就提示）----
if not defined DSH_HARNESS (
  if exist "D:\DEEPSEEK harness\deepseek-harness\apps\cli\lib\bin.js" set "DSH_HARNESS=D:\DEEPSEEK harness\deepseek-harness"
...
if not defined DSH_HARNESS (
  echo [错误] 未找到 dsh harness。
  echo        请设置 DSH_HARNESS 环境变量指向 harness checkout 根目录后重试。
  pause
  exit /b 1
)

REM ---- 3) 安装/更新皮肤到 web profile（幂等：已安装且指向本包则跳过）----
powershell -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%\ensure-skin.ps1" -SkinDir "%SKIN_DIR%"
if errorlevel 1 (
  echo [警告] 皮肤安装步骤未成功，将直接启动（皮肤可能不生效）。
)
